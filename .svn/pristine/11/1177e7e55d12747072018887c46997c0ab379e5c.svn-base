...
function checkLearnColumnRecord(
  ...
  return new Promise((resolve, reject) => {
    mainSqlite
      ...
      .then(result => {
        let now = utility.getNowDateTimeStr();
        if (result == 0) {
          mainSqlite
          ...
          mainSqlite
            .executeSql(
              `update column_schedule set updateTime='${now}', synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnType='${learnType}'`
            )
            .then(success => {
              _checkLearnRecord(studentNo, bookNo, columnNo, learnType).then(
                res => {
...
function _checkLearnRecord(studentNo, bookNo, columnNo, learnType = "DEFAULT") {
  return new Promise((resolve, reject) => {
    let nowDateTime = utility.getNowDateTimeStr();
    let nowDateStr = utility.getNowDateShortStr();
    mainSqlite
      .getTableDataCount(
        "learning_record",
...
// 初始化复习记录
function initBookReviewRecord(studentNo, bookNo, learnType = "DEFAULT") {
  return new Promise((resolve, reject) => {
    let nowDateTime = utility.getNowDateTimeStr();
    let nowDateStr = utility.getNowDateShortStr();
    mainSqlite
      .getTableDataCount(
        "review_record",
        `studentNo='${studentNo}' and bookNo='${bookNo}' and learnDate='${nowDateStr}' and learnType='${learnType}'`
      )
      .then(count => {
        if (count == 0) {
          mainSqlite
            .insertData("review_record", {
              studentNo: studentNo,
              bookNo: bookNo,
              learnType: learnType,
              learnDate: nowDateStr,
...
// 更新复习数据
function updateReviewData(reviewData, rightData, reviewedTime, reviewTimes) {
  let sqlArr = [];
  ...
  let reviewedNum = reviewData.length + rightData.length;

  // 3.更新review_record表
  if (reviewData[0]) {
    let studentNo = reviewData[0].studentNo;
    let learnType = reviewData[0].learnType;
    let bookNo = reviewData[0].bookNo;
    let nowDateStortStr = utility.getNowDateShortStr();
    // 3.更新复习表数据
    sqlArr.push(
      `update review_record set reviewedNum=reviewedNum+${reviewedNum},learnTimes=learnTimes+${reviewTimes}, learnedTime=learnedTime+${reviewedTime},synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and learnDate='${nowDateStortStr}' and learnType='${learnType}'`
    );
  }
...
function insertLearnData(
  ...
  if (studentNo) {
    ...
    let nowDateStortStr = utility.getNowDateShortStr();
    sqlArr.push(
      `update learning_record set rightNum=rightNum+${rightNum},errorNum=errorNum+${errorNum},learnTimes=learnTimes+${totalTimes}, learnedTime=learnedTime+${totalLearnedTime},synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnDate='${nowDateStortStr}' and learnType='${learnType}'`
    );
  }
...
function _getInsertRightSql(rightWords) {
  ...
  for (let columnNo in columnWordCount) {
    ...
    sqlArr.push(
      `update column_schedule set rightNum=rightNum+${rightCount}, updateTime='${now}', synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnType='${learnType}'`
    );
  }
  return sqlArr;
}
...
function _getInsertErrorSql(errWords) {
  ...
  for (let columnNo in columnWordCount) {
    let rightCount = columnWordCount[columnNo];
    sqlArr.push(
      `update column_schedule set errorNum=errorNum+${rightCount}, updateTime='${now}', synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnType='${learnType}'`
    );
  }
  return sqlArr;
}
...
function _getUpdateErrorSql(errWords) {
  ...
  errWords.forEach(word => {
    ...
    let now = utility.getNowDateTimeStr();
    sqlArr.push(
      `update column_schedule set synced=0, updateTime='${now}' where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnType='${learnType}'`
    );
  });
  return sqlArr;
}
...
// 插入拼写测试记录
function insertExamSpellRecord(examRecord, examDetails) {
  return new Promise((resolve, reject) => {
    mainSqlite.open().then(db => {
      db.transaction(
        tx => {
            ...
            tx.executeSql(
              ...
              (tx, results) => {
                ...
                examDetails.forEach(detailItem => {
                  ...
                  let keys = [];
                  let valArr = [];
                  entry.USER_SPELL_EXAM_DETAIL.forEach(examItem => {
                    let key = examItem.key;
                    keys.push(key);
                    let val = detailItem[key];
                    if (val === undefined) {
                  ...
                  let detailSql = `insert into exam_spell_details (${keys.join(
                    ","
                  )}) VALUES(${valArr.join(",")})`;
                  tx.executeSql(detailSql);
                });
...
//获取拼写测试详情表的数据
function getSpellExamDetails(id) {
  return new Promise((resolve, reject) => {
    mainSqlite.getTableData("exam_spell_details", `examId=${id}`).then(
      res => {
        resolve(res);
      },
      err => {
    ...
    mainSqlite
      ...
      .then(success => {
        let str = `update column_schedule set rightNum=rightNum+${
          missedWords.length
        }, synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnType='${learnType}'`;
...
// 处理异正确词表中常情况的数据
function updateColumnRightWords(
  studentNo,
  bookNo,
  columnNo,
  rightCount,
  ...
  let str = `update column_schedule set rightNum=${rightCount}, synced=0 where studentNo='${studentNo}' and bookNo='${bookNo}' and columnNo='${columnNo}' and learnType='${learnType}'`;
  return mainSqlite.executeSql(str);
}
...
export default {
  ...
  getAllExanRecord,
  updateExanRecord,
  insertExamSpellRecord,
  getSpellExamDetails,
  updateColumnRightWords,
  initBookReviewRecord
};
