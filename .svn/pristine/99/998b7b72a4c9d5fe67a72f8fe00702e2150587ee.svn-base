...
export default class extends React.Component {
  ...
  componentDidMount() {
    utility.getCurrUserInfo().then(item => {
      ...
      // AsyncStorage.getItem("serverTime").then(time => {
      //   //获取服务器时间
      //   if (time) {
      //     this.setState({
      //       serverTime: time
      ...
      //     let serverDate = utility.timeStrToObj(time);
    ...
    this.downloadListen = DeviceEventEmitter.addListener(
      "showAPKDownload",
      () => {
  ...
  componentWillUnmount() {
    ...
    this.rechargeListener && this.rechargeListener.remove();
    this.downloadListen && this.downloadListen.remove();
    soundManage.release();
  }
  showLoading() {
    ...
    return (
      <View style={{ height: "100%" }}>
        ...
        {/* <TimeChecker
          ...
          onSetted={() => {
            this.checkDownloadSync(this.studentNo);
          }}
        /> */}
