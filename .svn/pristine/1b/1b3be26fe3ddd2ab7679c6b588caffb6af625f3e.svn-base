...
export default class Test extends Component {
  ...
  computedScore(rightScore, errorScore, noAnswer) {
    let { typeScore, name, max, min } = this.params;
    let score = 0;
    let n = rightScore / this.maxNumber;
    if (n <= 0.1) {
      score = Math.ceil(min / 3 * n);
    } else if (n <= 0.9) {
      score = Math.ceil(min * n);
    } else {
      score = Math.ceil(max * n);
    }
    return score;
  }
  wordTestEnd(result) {
    ...
    let score = 0;
    if (rightScore !== 0) {
      score = this.computedScore(rightScore, errorScore, noAnswer);
    }
