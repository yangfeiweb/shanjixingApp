...
function getBookList() {
  return new Promise((resolve, reject) => {
    dataService.getBookList().then(
      res => {
        let code = res.code;
        if (code === 200) {
          let data = res.data.content;
          // 入库课本
          insertBooksToDB(data).then(res => {
            resolve(data);
          });
        } else {
          getBookListInDB().then(books => {
            resolve(books);
          });
        }
      },
...
function getUserBooksList(studentNo) {
  return new Promise((resolve, reject) => {
    booksDB.getUserBooksList(studentNo).then(
      books => {
        booksDB.getUserBooksSchedule(studentNo).then(
          schedules => {
...
function _insertColumns(
  ...
  _insertColumnWords(nextCol, version).then(
    ...
    err => {
      reject && reject();
      utility.log("--------insert..columns...err..", err);
