import { ToastAndroid, AsyncStorage, Dimensions } from "react-native";
import { ResourcePath } from "../services/urlConfig";
const oneDayTime = 24 * 60 * 60 * 1000; //一天的时间
const DEVICE_DEFAULT_HEIGHT = 750;

let deviceH = Dimensions.get("window").height;
let deviceRatio = deviceH / DEVICE_DEFAULT_HEIGHT;
...
function getTs() {
  return dateToTimeStr(getNowDate()).replace(/[\s|\s*-\s*|:]/g, "");
}
...
function getVipRemainDay() {
  return new Promise((resolve, reject) => {
    getCurrUserInfo().then(
      res => {
        if (res) {
          let day = computeDays(res.vipDeadTime, res.serverTime);
          resolve(day);
        } else {
          resolve(0);
        }
...
function setSeverTime(serverTimeStr) {
  let serverTime = timeStrToObj(serverTimeStr);
  let nowDate = new Date();
  dateOffset = serverTime - nowDate;
}

// 获取当前系统时间对象
function getNowDate() {
  let nowDateNum = new Date();
  return new Date(nowDateNum.valueOf() + dateOffset);
}

// 返回当前时间标准字符串 2018-05-14 15:33:20
function getNowDateTimeStr() {
  return dateToTimeStr(getNowDate());
}

// 返回当前时间标准字符串 2018-05-14
function getNowDateStr() {
  return dateToDateStr(getNowDate());
}

// 返回当前时间标准字符串 20180514
function getNowDateShortStr() {
  return dateToDateShortStr(getNowDate());
}
