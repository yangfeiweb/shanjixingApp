...
const TODAY = utility.getNowDateStr(); //当日的日期
export default class extends Component {
            ...
            if (currInfo) {
              ...
              if (currInfo.currentGrade) {
                index = GRADLE.findIndex(
                  item => item.key === currInfo.currentGrade
                );
              }
              name = currInfo.name || "";
  ...
  save() {
    ...
    this.loading && this.loading.show();
    dataService.changeUserInfo(obj).then(
      res => {
        let code = res.code;
        let msg = res.msg;
        if (code === 200) {
          ...
          AsyncStorage.getItem("usersInfo").then(res => {
            ...
            if (index !== -1) {
              ...
              infos.push(Object.assign(data, this.params));
        ...
        this.loadi && ngthis.loading.hide();
      },
      err => {
        this.loadi && ngthis.loading.hide();
      }
              ...
              <CardItem>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  <DatePicker
                    ...
                    showIcon={false}
                    onDateChange={date => {
                      this.params["birthDate"] = date.replace(/\s*-\s*/g, "");
