...
export default class extends React.Component {
  ...
  subDel() {
    // 判断是否已经同步
    let examItem = this.state.dataList[this.state.delIndex];
    utility.log("==========examItem...", examItem);
    let examId = examItem.id;
    if (examItem.synced === 1) {
      let cloudId = examItem.cloudId;
      if (cloudId) {
        dataService.deleteExamById(examId).then(res => {
          utility.log("--------deleteExamById", res);
          if (res.code === 200) {
            // 删除数据库中的试卷
            booksDB.deleteExamRecord(examId).then(success => {
              this.state.dataList.splice(this.state.delIndex, 1);
              this.setState({ dataList: this.state.dataList }, () => {});
              utility.Toast("删除试卷成功！");
              this.refs.delDialog.dismiss();
            });
          }
        });
      } else {
        dataService.deleteExamBySyncId(examId).then(result => {
          utility.log("--------deleteExamBySyncId", result);
          if (result.code === 200) {
            // 删除数据库中的试卷
            booksDB.deleteExamRecord(examId).then(success => {
              this.state.dataList.splice(this.state.delIndex, 1);
      ...
      return (
        <View style={styles.btnGroup}>
          <View style={{ flexDirection: "column", alignItems: "center" }}>
            <Text style={{ color: "#FB3E1A" }}>词汇量</Text>
            <Text style={{ color: "#FB3E1A" }}>{score} 个</Text>
          </View>
          <TouchableOpacity
            onPress={() => {
              utility.log(this.state.dataList[index]);
        ...
        >
          <View>
            ...
            <View style={styles.dialogBtn}>
              <Button block style={styles.sureBtn} onPress={this.subDel}>
